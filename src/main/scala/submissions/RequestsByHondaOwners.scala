...
object RequestsByHondaOnwers extends App {
  ...
  while (!quit) {
    if (scanner.hasNextInt()) {
      ...
      scanner.nextLine()
      println(s"${numTyped} is selected.")
  ...
  val percentage =
    ((sortedMap(carMakeTyped)).toFloat / (numOfTotalRequests).toFloat) * 100
  ...
  var values = Array(
    carMakeTyped,
    sortedMap(carMakeTyped).toString(),
    numOfTotalRequests.toString(),
    s"${percentage - (percentage % 0.01)}%"
